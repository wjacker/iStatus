...
建议替换为一张当前样式的详情弹窗截图，体现统一标题栏、单层结构和更新后的进程列表。
...
建议替换为一张当前版本的内存或磁盘详情截图，用来展示最新的环图与 breakdown 结构。
...
- `docs/screenshots/network-popover.png`
  替换为任意一个当前详情弹窗截图，突出统一弹窗头部和单层结构。
- `docs/screenshots/dashboard-overview.png`
  更新为当前 Overview 瀑布流布局，以及新版侧边栏/头部样式。
- `docs/screenshots/battery-panel.png`
  可替换为最新电池详情，或者改成一张内存/磁盘详情图。

另外这几张图也很值得补上：

- `docs/screenshots/menu-bar-settings.png`
  展示独立的菜单栏设置窗口，包括状态条预览和显示项开关。
- `docs/screenshots/memory-popover.png`
  展示当前紧凑版双环内存弹窗和新版 breakdown 面板。
- `docs/screenshots/disk-popover.png`
  展示当前单环磁盘弹窗和参考 Memory 风格的容量 breakdown。
- `docs/screenshots/cpu-temp-popover.png`
  如果你想突出温度、热压力和风扇遥测，这张也很适合补。
